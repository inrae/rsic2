...
get_xml_node_num_by_name <- function(
  name,
  ...
  if (is.na(node)) {
    stop(sprintf(
      "Node with name '%s' not found in the XML project file.",
      name
    ))
  }
...
#' @rdname get_xml_num_by_name
#' @export
get_xml_reach_num_by_name <- function(
  name,
  xp = xml2::read_xml(cfg$project$path),
  cfg
) {
  ...
  if (is.na(node)) {
    stop(sprintf(
      "Reach with name '%s' not found in the XML project file.",
      name
    ))
  }
  node_num <- xml2::xml_attr(node, "Num")
...
#' @rdname get_xml_num_by_name
#' @export
get_xml_section_num_by_name <- function(
  name,
  xp = xml2::read_xml(cfg$project$path),
  cfg
) {
  # Get the node number
  xPath <- sprintf(
    "/Reseau/Liste_Biefs/Bief/Liste_Sections/SectionMin[@Nom='%s']",
    name
  )
  node <- xp %>% xml2::xml_find_first(xPath)
  if (is.na(node)) {
    stop(sprintf(
      "Section with name '%s' not found in the XML project file.",
      name
    ))
  }
...
get_xml_reach_num_by_node_num <- function(
  node_num,
  upstream = TRUE,
  xp = xml2::read_xml(cfg$project$path),
  cfg
  ...
  xPath <- sprintf(
    "/Reseau/Liste_Biefs/Bief/Top/Noeud%s[text()=%i]",
    updown_string,
    as.integer(node_num)
  )
  ...
  if (is.na(node)) {
    stop(sprintf(
      "Reach with node number '%d' not found in the XML project file.",
      node_num
    ))
  }
  reach_num <- node |>
